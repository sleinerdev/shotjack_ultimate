...
function sendModal(m,pid,youDrink,distribute,sanction){
  ...
  const conn=[...conns].find(c=>c.playerId===pid && c.matchId===m.id);
...
function onAck(m,pid,kind){
  ...
  if(kind==="distribute" && m.pending.needDistributeAck.has(pid)){
    // Calculer et ajouter les gorgées distribuées
    const p=m.players[pid]; 
    if(p) {
      let distribute=0;
      for(const h of p.hands){
...
wss.on("connection",(ws)=>{
  ...
  ws.on("message", async (buf)=>{
    ...
    if(t==="leave_match"){
      ...
      // Note: pas de send("left") car le client gère lui-même leftRef
      return;
    }
